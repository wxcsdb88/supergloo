...
var _ = Describe("InstallClient", func() {
	...
	for _, test := range []typed.ResourceClientTester{
		...
		Context("resource client backed by "+test.Description(), func() {
			...
			It("CRUDs Installs", func() {
				InstallClientTest(namespace, client)
			})
		})
	}
})

func InstallClientTest(namespace string, client InstallClient) {
	err := client.Register()
	Expect(err).NotTo(HaveOccurred())
	...
	Expect(r1.Enabled).To(Equal(input.Enabled))
