...
var _ = Describe("RbacConfigClient", func() {
	...
	for _, test := range []typed.ResourceClientTester{
		...
		Context("resource client backed by "+test.Description(), func() {
			...
			It("CRUDs RbacConfigs", func() {
				RbacConfigClientTest(namespace, client)
			})
		})
	}
})

func RbacConfigClientTest(namespace string, client RbacConfigClient) {
	err := client.Register()
	Expect(err).NotTo(HaveOccurred())
	...
	Expect(r1.Exclusion).To(Equal(input.Exclusion))
	Expect(r1.EnforcementMode).To(Equal(input.EnforcementMode))
