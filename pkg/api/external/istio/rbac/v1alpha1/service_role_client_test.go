...
var _ = Describe("ServiceRoleClient", func() {
	var (
		namespace string
	)
	for _, test := range []typed.ResourceClientTester{
		&typed.KubeRcTester{Crd: ServiceRoleCrd},
		...
		Context("resource client backed by "+test.Description(), func() {
			...
			BeforeEach(func() {
				...
				client, err = NewServiceRoleClient(factory)
				Expect(err).NotTo(HaveOccurred())
			})
			AfterEach(func() {
				test.Teardown(namespace)
			})
			It("CRUDs ServiceRoles", func() {
				ServiceRoleClientTest(namespace, client)
			})
		})
	}
})

func ServiceRoleClientTest(namespace string, client ServiceRoleClient) {
	err := client.Register()
	Expect(err).NotTo(HaveOccurred())

	name := "foo"
	input := NewServiceRole(namespace, name)
	...
	Expect(r1).To(BeAssignableToTypeOf(&ServiceRole{}))
