...
var _ = Describe("ServiceRoleBindingClient", func() {
	var (
		namespace string
	)
	for _, test := range []typed.ResourceClientTester{
		&typed.KubeRcTester{Crd: ServiceRoleBindingCrd},
		...
		Context("resource client backed by "+test.Description(), func() {
			...
			BeforeEach(func() {
				...
				client, err = NewServiceRoleBindingClient(factory)
				Expect(err).NotTo(HaveOccurred())
			})
			AfterEach(func() {
				test.Teardown(namespace)
			})
			It("CRUDs ServiceRoleBindings", func() {
				ServiceRoleBindingClientTest(namespace, client)
			})
		})
	}
})

func ServiceRoleBindingClientTest(namespace string, client ServiceRoleBindingClient) {
	err := client.Register()
	Expect(err).NotTo(HaveOccurred())

	name := "foo"
	input := NewServiceRoleBinding(namespace, name)
	...
	Expect(r1).To(BeAssignableToTypeOf(&ServiceRoleBinding{}))
	...
	Expect(r1.Subjects).To(Equal(input.Subjects))
	Expect(r1.RoleRef).To(Equal(input.RoleRef))
